        ...
        {members.map((member, i) => (
          <div key={member.name} className="bg-[#151a21] rounded-xl overflow-hidden">
            <div className="aspect-square w-full bg-[#1a1f26] flex items-center justify-center">
              {member.photo ? (
                <img src={member.photo} alt={member.name} className="w-full h-full object-cover" style={{ objectPosition: member.objectPosition ?? "top" }} />
              ) : (
    ...
    >
      {member.photo ? (
        <Image
          ...
          style={{ filter: isActive ? "grayscale(0) brightness(1)" : "grayscale(0) brightness(0.85)", objectPosition: member.objectPosition ?? "top" }}
        />
      ) : (
